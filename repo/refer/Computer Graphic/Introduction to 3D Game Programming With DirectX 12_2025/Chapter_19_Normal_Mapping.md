...
So how do we compress a unit vector into this format? First note that for a unit vector, each coordinate always lies in the range [鈥?, 1]. If we shift and scale this range to [0, 1] and multiply by 255 and truncate the decimal, the result will be an integer in the range 0鈥?55. That is, if $x$ is a coordinate in the range [鈥?, 1], then the integer part of ${ \dot { f } } ( x )$ is an integer in the range 0鈥?55, where $f$ is defined by 
...
The next question is how to reverse the compression process; that is, given a compressed texture coordinate in the range 0鈥?55, how can we recover its true value in the interval [鈥?, 1]. The answer is to simply invert the function $f ,$ which after a little thought, can be seen to be: 

$$
f ^ {- 1} (x) = \frac {2 x}{2 5 5} - 1
$$

That is, if $x$ is an integer in the range 0鈥?55, then $f ^ { - 1 } ( x )$ is a floating-point number in the range [鈥?, 1]. 
...
Thus, the Sample method has already done part of the uncompressing work for us (namely the divide by 255, which transforms an integer in the range 0鈥?55 to the floating-point interval [0, 1]). We complete the transformation by shifting and scaling each component in [0, 1] to [鈥?, 1] with the function $g \colon [ 0 , 1 ]  [ - 1 , 1 ]$ defined by: 
...
This works because the scalar 1.0 is augmented to the vector (1, 1, 1) so that the expression makes sense and is done componentwise. You can also use a signed SNORM format to store normals such as DXGI_FORMAT_R8G8B8A8_SNORM so that you do not need to remap from [0, 1] to [鈥?, 1]. 
...
In the above, we used the fact that the inverse of a matrix $\mathbf { A } = { \left[ \begin{array} { l l } { a } & { b } \\ { c } & { d } \end{array} \right] }$ 铮筨 is given by: 
...
Once we have the normal from the normal map, which we call the 鈥渂umped normal,鈥?we use it for all the subsequent calculation involving the normal vector (e.g., lighting, cube mapping). The entire normal mapping effect is shown below for completeness, with the parts relevant to normal mapping in bold. 
...
float4 directLight $=$ ComputeLighting(gLights,mat锛宲in(PosW, bumpedNormalW,toEyeW锛宻hadowFactor);   
float4 litColor $=$ ambient $^+$ directLight;   
//Add in specular reflections.   
if( gReflectionsEnabled锛墈 TextureCube gCubeMap $=$ ResourceDescriptorHeap[gSkyBoxIndex]; float3r $=$ reflect(-toEyeW,bumpedNormalW); float4reflectionColor $\equiv$ gCubeMap. Sample(GetLinearWrapSampler(),r); float3fresnelFactor $=$ SchlickFresnel(fresnelR0, bumpedNormalW,r); litColor.rgb $+ =$ ambientAccess\*shininess \*fresnelFactor\* reflectionColor.rgb;   
}   
//Common convention to take alpha from diffuse albedo. litColor.a $=$ diffuseAlbedo.a; return litColor; 

Observe that the 鈥渂umped normal鈥?vector is use in the light calculation, but also in the reflection calculation for modeling reflections from the environment map. In addition, we introduce a new texture map associated with each material. 
...
The scalar $h \in [ 0 , 1 ]$ is the scalar value obtained from the heightmap. We subtract 1 from $h$ to shift the interval $[ 0 , 1 ]  [ - 1 , 0 ]$ ; because the surface normal is outward facing from the mesh, this means that we displace 鈥渋nward鈥?instead of 鈥渙utward.鈥?This is common convention, as it is usually more convenient to 鈥減op鈥?geometry in rather than 鈥減op鈥?geometry out. The scalar $s$ is a scale factor to scale the overall height to some world space value. The height value $h$ from the heightmap is in a 
...
normalized range where 0 means the lowest height (most inward displacement) and 1 means the highest height (no displacement). Let鈥檚 say we want to, at most, offset by 5 world units. Then we take $s = 5$ so that $s ( h - 1 ) \in [ - 5 , 0 ]$ . 
...
{ float d $=$ distance(p,gEyePosW); float s $=$ saturate((d-gMeshMinTessDist)/(gMeshMaxTessDist-gMeshMinTessDist)); return pow(2,(lerp(gMeshMaxTess,gMeshMinTess,s))锛?   
} 
...
Recall that the control point hull shader inputs the control points for a patch and outputs the control points for a patch (possibly different from the input patch size). The control point hull shader is invoked once per control point output but has access to the entire input patch. In our case, we input a triangle patch and output a triangle patch, and the control point hull shader is simply a 鈥減ass through鈥?shader: 
...
c) Create a new 鈥渙cean鈥?material to give it a bluish tint, but still keep some reflection in from the environment map. 
