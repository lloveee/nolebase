...
>
> Bevy = Rust 编写的现代化 ECS（Entity-Component-System）游戏引擎，2020 年至今，5.x 版本。
> **80k+ GitHub stars**，是 Rust 游戏开发的"事实标准"。
...
| **生命周期阶段** | Plugin::build → Plugin::ready → ... | Created → Starting → Running → Stopping → Stopped/Failed |
...
| **服务定位** | Query<T> | ctx.GetCapability<T>(key) |
| **事件总线** | `EventWriter<MyEvent>` / `EventReader<MyEvent>` | `router.Emit(diagnostic)` + future event bus |
| **多实例** | 多个 World | 多个 Session |
...
| **生态成熟度** | ✅ 80k+ stars, 大量 plugin | 0 | Bevy 更好 |
...
这两个参照系让 spec 的设计选择**可被验证、可被质疑、可被借鉴**。后续 Context Runtime 实现时，可以再加一篇 `09-Context-Runtime-Implementation.md` 来讲实际的算法实现。