...
**为什么 ModApi 不依赖 Context？** 因为 Mod 作者**不应该**看到 Context Runtime 的具体类型——spec 第 6.1 节："Public contracts, replaceable implementations"。
...
**如果 Mod 想注册能力**——它应该通过 Context Runtime 的注册方法（`RegisterCapability<T>(...)`），而**不是**通过 IModuleContext 拿一个万能解析器。**当前接口没有暴露这些注册方法**——因为 Context Runtime 还没实现，未来添加时**会扩展这个接口（或加新接口）**，但不会改 IModule 的形状。
...
3. **`IModuleContext` 接口故意薄**——未来加 `RegisterCapability<T>` 等方法时，**接口会变**——是 breaking change。spec 第 11.2 节"explicit compatibility"原则会要求老 Mod 升级后重编译。
4. **没有 `GetCapability<T>(key)`**——Context Runtime 实现时大概率要加，类型签名需要仔细设计（是同步还是异步？返回 `Result<T>` 还是直接抛？）。
...
- [Spec 6.1 节"Stable contracts"](https://en.wikipedia.org/wiki/Design_by_contract)
